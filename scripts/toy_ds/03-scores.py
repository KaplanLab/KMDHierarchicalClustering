...
if len(sys.argv) < 2:
    ...
    print("CSV_FILE should have the format DATASET_pred_X for some X and some DATASET.csv file")
    sys.exit(1)

files = sys.argv[1:]
ds_names = [ re.sub("_pred.*", "", x) for x in files ]
ds_datafiles = [ f"{x}.csv" for x in ds_names ]
...
for y_file, ds_file in zip(files, ds_datafiles):
    ...
    y_true = pd.read_csv(ds_file)['label'].to_numpy()
