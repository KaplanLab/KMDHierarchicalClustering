### 
# Clone https://github.com/10XGenomics/single-cell-3prime-paper, then
# PUT THIS FILE INSTEAD OF single-cell-3prime-paper/pbmc68k_analysis/main_process_pure_pbmc.R
# THEN RUN IT FROM THE CURRENT DIRECTORY
# ALTERNATIVELY, CHANGE PROG_DIR AND DATA_DIR BELOW TO RUN IT FROM ANOTHER DIRECTORY
###
...
FIG_DIR <- NULL #"/srv01/technion/hagaik/storage/aviv/pbmc_pure/"        # SPECIFY HERE
...
subsampled_purified_mats <- lapply(1:length(all_data), function(i) { # subsample the matrix to match tgt_rpc
  cat(sprintf("%d...\n", i))
  a = .downsample_gene_bc_mtx(all_json[[i]], all_data[[i]], all_mol_info[[i]], tgt_rpc, 'conf_mapped_reads')[[1]]
  rownames(a) <- all_data[[i]]$hg19$barcodes
  colnames(a) <- all_data[[i]]$hg19$genes
  return(a)
} )
cat(colnames(subsampled_purified_mats[[1]]))
# -------------------------------------------
cat("run PCA and tSNE on down-sampled data\n")
...
# ------------------------------------
cat("curate the purified populations\n")
# ------------------------------------
...
pure_select_11<-lapply(1:length(sub_idx),function(i) {
					   subsampled_purified_mats[[sub_idx[[i]]$sample[1]]][sub_idx[[i]]$use,]}

)
saveRDS(pure_select_11,file=file.path(DATA_DIR, "all_pure_select_11.rds"))
# -------------------------------------------------
